...
import com.nearbite.mapper.VoucherOrderMapper;
import com.nearbite.service.ISeckillVoucherService;
import com.nearbite.service.IVoucherOrderService;
import com.nearbite.utils.RedisIdWorker;
import com.nearbite.utils.UserHolder;
...
public class VoucherOrderServiceImpl extends ServiceImpl<VoucherOrderMapper, VoucherOrder> implements IVoucherOrderService {
    ...
    private final String consumerName = "c-" + UUID.randomUUID().toString(true);
    ...
    private void createConsumerGroupIfNecessary() {
        try {
            stringRedisTemplate.execute((RedisCallback<String>) connection -> {
                byte[] key = stringRedisTemplate.getStringSerializer().serialize(STREAM_ORDERS_KEY);
                return connection.xGroupCreate(key, STREAM_ORDERS_GROUP, ReadOffset.from("0"), true);
            });
        } catch (RuntimeException e) {
            // 已存在的消费组可以直接复用；其它异常（例如连接失败）应阻止应用带病启动
        ...
        public void run() {
            while (true) {
                try {
                    ...
                    // 4.确认消息 XACK
                    stringRedisTemplate.opsForStream().acknowledge(STREAM_ORDERS_KEY, STREAM_ORDERS_GROUP, record.getId());
                } catch (Exception e) {
                    log.error("处理订单异常", e);
                    handlePendingList();
                }
    ...
    private void createVoucherOrder(VoucherOrder voucherOrder) {
        ...
        try {
            transactionTemplate.execute(status -> {
                // 数据库唯一索引是最终兜底，这里的查询用于快速识别重复订单
                int count = query().eq("user_id", userId).eq("voucher_id", voucherId).count();
                if (count > 0) {
                    log.warn("用户 {} 已购买优惠券 {}，忽略重复消息", userId, voucherId);
                    return null;
                }
                ...
                boolean success = seckillVoucherService.update()
                        ...
                        .update();
                if (!success) {
                    log.warn("优惠券 {} 库存不足，订单 {} 未创建", voucherId, voucherOrder.getId());
                    return null;
                }

                if (!save(voucherOrder)) {
                    throw new IllegalStateException("创建订单失败：" + voucherOrder.getId());
                }
                return null;
            });
        } finally {
